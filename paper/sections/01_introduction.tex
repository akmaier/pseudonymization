...
replaces an identifier with a realistic \emph{surrogate}; \emph{anonymisation} prevents singling
out. Stable pseudonyms preserve longitudinal utility but hand an adversary a persistent handle.
European guidance treats keyed message authentication codes as
...
combine technical performance, information loss and an intruder test~\cite{mozes2021nointruder},
while surrogate systems and masking benchmarks cover single
pieces~\cite{eder2019emails,pilan2022tab,osborne2022bratsynthetic}. None states together what one
policy buys and what it costs. Data-protection officers release documents and cases, not average
tokens, and one unchanged mention can identify someone after thousands were removed, so we measure
surviving findings, exposed cases and linkable entities rather than token rates alone.

One design covers four corpora in German, English, Chinese and Arabic: clinical letters, legal
judgments, news and other genres, and e-mail. Every document receives the same detector comparison,
release conditions and measurements. Two predictions are put at risk: that detector precision
governs utility more strongly than the choice between surrogates and typed placeholders, and that
detection recall moves the two attacks in opposite directions, because removing a name obstructs
context linkage while leaving a larger, more stable population for frequency matching. The clinical
corpus has a known weakness: its gold names were inserted into an already de-identified release, so
its name distribution is synthetic.
