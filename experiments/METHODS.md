...
> **Status, 2026-09-26.** Written 2026-09-08, before the engine was built, and kept as the design
> record: it says why the engine has the shape it has, and what was measured to decide that. The
> specification is [`experiment_plan.md`](../experiment_plan.md), and where the two disagree that
> file governs. This document was never under version control until today; no original text has
> been removed in committing it. Three passages still describe the study as it was planned rather
> than as it ran, and are marked inline.
...
`experiment_plan.md` §Factors is the specification; nothing here changes it.
...
| **raw surface form** | *Dr. Weber*, *Weber*, *F. Weber* are three keys → three pseudonyms. This is what a real deployed system does, and it is exactly the failure `experiment_plan.md` §Measurements/2 wants to quantify |
...
| CARDIO:DE, ~~BRONCO~~ | medication IE, section classes, ICD/OPS/ATC | zero-shot | F1 |

> BRONCO150 never arrived — no answer to the request since 2026-09-07 — so the clinical
> utility arm is CARDIO:DE alone.
...
> Run. A5 is reported in the paper alongside A3, on Enron and CARDIO:DE; the corpora where
> co-reference is document-scoped cannot support it.
